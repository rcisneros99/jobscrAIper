...
def compress_and_resize_image(image_path, output_size=(256, 256), quality=30):
    """
    ...
    Args:
        ...
        quality (int): JPEG quality for saving the compressed image.
        
    Returns:
        str: Path to the compressed and resized image.
    """
    # Open the image file
    ...
    img = img.convert("RGB")
    # Define output path
    compressed_image_path = image_path.replace(".png", "_compressed.jpg")
...
def clip_message_and_obs(msg, max_img_num):
    """
    Clips the message history to include a maximum number of images.
    Parameters:
    - msg: The message history.
    - max_img_num: The maximum number of images to include.
    Returns:
    - clipped_msg: The clipped message history.
    """
    clipped_msg = []
    img_num = 0
    for idx in range(len(msg)):
        ...
        if curr_msg['role'] != 'user':
            ...
            # Since we no longer include base64 images in 'content',
            # we can include the messages as is
            if img_num < max_img_num:
                img_num += 1
                clipped_msg = [curr_msg] + clipped_msg
            else:
                # Omit the screenshot details to save tokens
                msg_no_pdf = curr_msg['content'].split("Observation:")[0].strip() + " Observation: Screenshot omitted to save space."
                curr_msg_clip = {
                    'role': curr_msg['role'],
                    'content': msg_no_pdf
                }
...
def print_message(json_object, save_dir=None):
    """
    Prints and saves the message history.
    Parameters:
    - json_object: The message history.
    - save_dir: Directory to save the messages (optional).
    """
...
def extract_information(text):
    patterns = {
        ...
        "goback": r"GoBack",
        "google": r"Google",
        "extractjobinfo": r"ExtractJobInfo"
    }

    for key, pattern in patterns.items():
        match = re.search(pattern, text)
        if match:
            if key in ["click", "wait", "goback", "extractjobinfo"]:
                # no content
...
def get_pdf_retrieval_ans_from_assistant(api, pdf_path, task):
    ...
    message = api.beta.threads.messages.create(
        thread_id=thread.id,
        role="user",
        content=task,
        file_ids=[file.id]
    ...
    while True:
        ...
        run_status = api.beta.threads.runs.retrieve(thread_id=thread.id, run_id=run.id)
        if run_status.status == 'completed':
            break
        time.sleep(2)
    messages = api.beta.threads.messages.list(thread_id=thread.id)
    messages_text = messages.data[0].content[0].text.value
    file_deletion_status = api.beta.assistants.files.delete(
        assistant_id=assistant.id,
        file_id=file.id
    )
    # print(file_deletion_status)
    logging.info(file_deletion_status)
    assistant_deletion_status = api.beta.assistants.delete(assistant.id)
    # print(assistant_deletion_status)
    logging.info(assistant_deletion_status)
    return messages_text


def upload_image_and_get_presigned_url(file_path, s3_client, expiration=300):

    """Uploads an image to S3 and generates a pre-signed URL."""
    object_name = f"temp_images/{datetime.now().strftime('%Y%m%d%H%M%S')}_screenshot.png"
    
    try:
        # Upload the file to S3
        s3_client.upload_file(file_path, BUCKET_NAME, object_name)
        
        # Generate a pre-signed URL that expires in 1 hour by default
